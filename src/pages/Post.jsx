...
function Comment({ username, profilePicture, comment, _id, time }) {
  ...
  return (
    <div className="flex gap-3 items-center mb-4">
      ...
      <button type="button" className="text-2xl relative" onClick={()=>{
        setOptionsOpen((prev)=>!prev)
      }}>
        <BiDotsHorizontalRounded />
     {optionsOpen && <button type="button" className="absolute px-6 py-2 right-0 text-base bg-[#f3f2f2] rounded-lg" onClick={()=>{ 
          console.log("hello")
          setDailog({
...
export default function Post() {
  ...
  return (
    <section
      id="Post"
      className="h-screen w-screen fixed top-0 left-0 bg-[rgba(0,0,0,0.5)] grid place-items-center px-4 py-20 md:p-4 z-50 overflow-y-auto"
    >
      <div className=" w-full md:w-3/4 lg:w-[45%] bg-white  rounded-xl py-6 px-4  flex flex-col gap-5 md:py-8 md:px-6">
        ...
        <div id="comment-section">{comments.map((item,index)=><Comment key={index} {...item}/>)}</div>
      </div>
    </section>
  );
}
